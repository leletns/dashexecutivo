  ...
  React.useEffect(() => {
    ...
    const load = () => {
      ...
      fetch(url, { cache: "no-store" })
        ...
        .then((d) => {
          // Diagnóstico silencioso no console do navegador (sem nada visível no
          // painel) — ajuda a entender uma eventual divergência sem poluir a UI.
          if (d?.fonte || d?.aviso || d?.error) {
